class Admin::PersonsController < Admin::BaseController
  ...
  def load_person
    @person = account.persons.find(params[:id])
  end
  

  def load_all_persons
    @persons = account.persons.page(params[:page])
  end
  ...
  def update
    ...
    if @person.update(modify_date_inputs_on_params(person_params))
      flash[:notice] = (flash[:notice] || "") << "Saved #{@person.name}"
    else
      flash[:alert] = (flash[:alert] || "") << "Person could not be saved."
